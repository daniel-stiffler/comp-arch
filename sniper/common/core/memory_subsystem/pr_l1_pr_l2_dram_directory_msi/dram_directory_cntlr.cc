...
#define MYLOG(...)                                                             \
  {                                                                            \
    ScopedLock l(iolock);                                                      \
    fflush(stdout);                                                            \
    printf("[%s] %d%cdd %-25s@%3u: ",                                          \
           itostr(getShmemPerfModel()->getElapsedTime()).c_str(),              \
           getMemoryManager()->getCore()->getId(),                             \
           Sim()->getCoreManager()->amiUserThread() ? '^' : '_', __FUNCTION__, \
           __LINE__);                                                          \
    printf(__VA_ARGS__);                                                       \
    printf("\n");                                                              \
    fflush(stdout);                                                            \
...
DramDirectoryCntlr::DramDirectoryCntlr(
    core_id_t core_id, MemoryManagerBase* memory_manager,
    AddressHomeLookup* dram_controller_home_lookup,
    UInt32 dram_directory_total_entries, UInt32 dram_directory_associativity,
    UInt32 cache_block_size, UInt32 dram_directory_max_num_sharers,
    UInt32 dram_directory_max_hw_sharers, String dram_directory_type_str,
  ...
  m_dram_directory_cache = new DramDirectoryCache(
      core_id, dram_directory_type_str, dram_directory_total_entries,
      dram_directory_associativity, cache_block_size,
      dram_directory_max_hw_sharers, dram_directory_max_num_sharers,
      dram_directory_cache_access_time, m_shmem_perf_model);
  m_dram_directory_req_queue_list = new ReqQueueList();
  for (DirectoryState::dstate_t state = DirectoryState::dstate_t(0);
       state < DirectoryState::NUM_DIRECTORY_STATES;
       state = DirectoryState::dstate_t(int(state) + 1)) {
    if (state != DirectoryState::UNCACHED) {
      evict[state] = 0;
      registerStatsMetric("directory", core_id,
                          String("evict-") + DStateString(state),
                          &evict[state]);
    }
  }
  ...
  if (protocol == "msi") {
    ...
    LOG_PRINT_ERROR("Invalid coherency protocol %s, must be msi, mesi or mesif",
                    protocol.c_str());
  }
}
...
void DramDirectoryCntlr::handleMsgFromL2Cache(core_id_t sender,
                                              ShmemMsg* shmem_msg) {
  ShmemMsg::msg_t shmem_msg_type = shmem_msg->getMsgType();
  SubsecondTime msg_time =
      getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD);
  IntPtr address = shmem_msg->getAddress();

  MYLOG("begin for address %lx, %d in queue", address,
        m_dram_directory_req_queue_list->size(address));

  // Look up line state in the tag directory
  // This is just for modeling the TD lookup time (this is the only place where
  // we call getDirectoryEntry with modeled==true),
  // elsewhere we assume outstanding requests are stored in a fast MSHR-like
  // structure
  ...
  switch (shmem_msg_type) {
    case ShmemMsg::EX_REQ: {
      ...
      if (m_dram_directory_req_queue_list->size(address) == 1) {
        ...
        MYLOG(
            "E REQ (%lx) not handled yet because of outstanding request in the "
            "queue",
            address);
      }
      break;
    ...
    case ShmemMsg::SH_REQ: {
      ...
      if (m_dram_directory_req_queue_list->size(address) == 1) {
        ...
        MYLOG(
            "S REQ (%lx) not handled because of outstanding request in the "
            "queue",
            address);
      }
      break;
    ...
    case ShmemMsg::UPGRADE_REQ:
      ...
      if (m_dram_directory_req_queue_list->size(address) == 1) {
        ...
        MYLOG(
            "UPGRADE REQ (%lx) not handled because of outstanding request in "
            "the queue",
            address);
      }
...
void DramDirectoryCntlr::handleMsgFromDRAM(core_id_t sender,
                                           ShmemMsg* shmem_msg) {
  MYLOG("Start");
  ShmemMsg::msg_t shmem_msg_type = shmem_msg->getMsgType();
...
void DramDirectoryCntlr::processNextReqFromL2Cache(IntPtr address) {
  MYLOG("Start processNextReqFromL2Cache(%lx): %d in Queue", address,
        m_dram_directory_req_queue_list->size(address));
  ...
  if (!m_dram_directory_req_queue_list->empty(address)) {
    ...
    getShmemPerfModel()->setElapsedTime(ShmemPerfModel::_SIM_THREAD,
                                        shmem_req->getTime());
    ...
    if (shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::EX_REQ) {
      ...
      LOG_PRINT_ERROR("Unrecognized Request(%u)",
                      shmem_req->getShmemMsg()->getMsgType());
  }
  MYLOG("End processNextReqFromL2Cache(%lx)", address);
}

DirectoryEntry* DramDirectoryCntlr::processDirectoryEntryAllocationReq(
    ShmemReq* shmem_req) {
  ...
  m_dram_directory_cache->getReplacementCandidates(address,
                                                   replacement_candidate_list);

  std::vector<DirectoryEntry*>::iterator it;
  std::vector<DirectoryEntry*>::iterator replacement_candidate =
      replacement_candidate_list.end();
  for (it = replacement_candidate_list.begin();
    ...
    if (((replacement_candidate == replacement_candidate_list.end()) ||
         ((*replacement_candidate)->getNumSharers() >
          (*it)->getNumSharers())) &&
  ...
  LOG_ASSERT_ERROR(replacement_candidate != replacement_candidate_list.end(),
                   "Cannot find a directory entry to be replaced with a "
                   "non-zero request list (see Redmine #175)");

  DirectoryState::dstate_t curr_dstate =
      (*replacement_candidate)->getDirectoryBlockInfo()->getDState();
  evict[curr_dstate]++;
  ...
  DirectoryEntry* directory_entry =
      m_dram_directory_cache->replaceDirectoryEntry(replaced_address, address,
                                                    true);

  ShmemMsg nullify_msg(ShmemMsg::NULLIFY_REQ, MemComponent::TAG_DIR,
                       MemComponent::TAG_DIR, requester, replaced_address, NULL,
                       0, NULL);
...
void DramDirectoryCntlr::processNullifyReq(ShmemReq* shmem_req) {
  IntPtr address      = shmem_req->getShmemMsg()->getAddress();
  core_id_t requester = shmem_req->getShmemMsg()->getRequester();

  // NOTE: no ShmemPerf accounting for nullify requests as they may happen
  // *after* core requests are completed
  ...
  switch (curr_dstate) {
    ...
    case DirectoryState::MODIFIED:
      getMemoryManager()->sendMsg(
          ShmemMsg::FLUSH_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
          requester /* requester */, directory_entry->getOwner() /* receiver */,
          address, NULL, 0, HitWhere::UNKNOWN, NULL,
    ...
    {
      std::pair<bool, std::vector<SInt32> > sharers_list_pair =
          directory_entry->getSharersList();
      if (sharers_list_pair.first == true) {
        ...
        getMemoryManager()->broadcastMsg(
            ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
            requester /* requester */, address, NULL, 0, NULL,
            ShmemPerfModel::_SIM_THREAD);
      } else {
        ...
        for (UInt32 i = 0; i < sharers_list_pair.second.size(); i++) {
          getMemoryManager()->sendMsg(
              ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
              requester /* requester */,
              sharers_list_pair.second[i] /* receiver */, address, NULL, 0,
              HitWhere::UNKNOWN, NULL, ShmemPerfModel::_SIM_THREAD);
        }
      }
    } break;
...
void DramDirectoryCntlr::processExReqFromL2Cache(ShmemReq* shmem_req,
  ...
  switch (curr_dstate) {
    case DirectoryState::EXCLUSIVE:  // Cache may have done a silent upgrade to
                                     // MODIFIED, so send a FLUSH (dirty data)
                                     // rather than an INV (data clean)
    case DirectoryState::MODIFIED: {
      ...
      getMemoryManager()->sendMsg(
          ShmemMsg::FLUSH_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
          requester /* requester */, directory_entry->getOwner() /* receiver */,
          address, NULL, 0, HitWhere::UNKNOWN,
          shmem_req->getShmemMsg()->getPerf(), ShmemPerfModel::_SIM_THREAD);
      break;
    }
    ...
    case DirectoryState::SHARED: {
      ...
      if (sharers_list_pair.first == true) {
        ...
        getMemoryManager()->broadcastMsg(
            ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
            requester /* requester */, address, NULL, 0,
    ...
    case DirectoryState::UNCACHED: {
      ...
      bool add_result = directory_entry->addSharer(
          requester, m_dram_directory_cache->getMaxHwSharers());
      ...
      retrieveDataAndSendToL2Cache(ShmemMsg::EX_REP, requester, address,
                                   cached_data_buf, shmem_req->getShmemMsg());
      break;
    }
...
void DramDirectoryCntlr::processShReqFromL2Cache(ShmemReq* shmem_req,
                                                 Byte* cached_data_buf) {
  IntPtr address      = shmem_req->getShmemMsg()->getAddress();
  core_id_t requester = shmem_req->getShmemMsg()->getRequester();
  ...
  switch (curr_dstate) {
    case DirectoryState::EXCLUSIVE: {
      ...
      assert(cached_data_buf == NULL);
      getMemoryManager()->sendMsg(
          ShmemMsg::WB_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
          requester /* requester */, directory_entry->getOwner() /* receiver */,
          address, NULL, 0, HitWhere::UNKNOWN,
          shmem_req->getShmemMsg()->getPerf(), ShmemPerfModel::_SIM_THREAD);
      break;
    ...
    case DirectoryState::SHARED: {
      if (directory_entry->hasSharer(requester)) {
        MYLOG(
            "got a WB/INV REP from the forwarder and now handling the original "
            "SH REQ");
        ++forward;
        if (cached_data_buf == NULL) {
          // Forwarder evicted the data while we requested it. Will have to get
          // it from DRAM anyway.
          ++forward_failed;
        }
        ...
        if (add_result == false) {
          ...
          MYLOG("INV_REQ>%d for %lx because I could not add sharer",
                directory_entry->getOwner(), address)
          getMemoryManager()->sendMsg(
              ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
              requester /* requester */, sharer_id /* receiver */, address,
              NULL, 0, HitWhere::UNKNOWN, shmem_req->getShmemMsg()->getPerf(),
              ShmemPerfModel::_SIM_THREAD);
        } else {
...
void DramDirectoryCntlr::retrieveDataAndSendToL2Cache(
    ShmemMsg::msg_t reply_msg_type, core_id_t receiver, IntPtr address,
  ...
  if (cached_data_buf != NULL) {
    ...
    getMemoryManager()->sendMsg(
        reply_msg_type, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
        receiver /* requester */, receiver /* receiver */, address,
        cached_data_buf, getCacheBlockSize(),
        HitWhere::
            CACHE_REMOTE /* cached_data_buf was filled by a WB_REQ or FLUSH_REQ */,
        orig_shmem_msg->getPerf(), ShmemPerfModel::_SIM_THREAD);
    ...
    if (m_protocol == CoherencyProtocol::MESIF &&
        shmem_req->getForwardingFrom() ==
            INVALID_CORE_ID  // If forwarding already failed once, don't try
                             // again
        &&
      ...
      getMemoryManager()->sendMsg(
          ShmemMsg::WB_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
          receiver /* requester */, forwarder /* receiver */, address, NULL, 0,
          HitWhere::UNKNOWN, shmem_req->getShmemMsg()->getPerf(),
          ShmemPerfModel::_SIM_THREAD);
      return;
    }
    ...
    if (shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::UPGRADE_REQ) {
      MYLOG(
          "Have to get data from DRAM while doing an UPGRADE_REQ: lost data "
          "somehow\n");
    }

    // Remember that this request is waiting for data, and should not be woken
    // up by voluntary invalidates
    ...
    getMemoryManager()->sendMsg(
        ...
        MemComponent::TAG_DIR, MemComponent::DRAM, receiver /* requester */,
        dram_node /* receiver */, address, NULL, 0, HitWhere::UNKNOWN,
        orig_shmem_msg->getPerf(), ShmemPerfModel::_SIM_THREAD);
  }
  MYLOG("End @ %lx", address);
}

void DramDirectoryCntlr::processDRAMReply(core_id_t sender,
                                          ShmemMsg* shmem_msg) {
  IntPtr address = shmem_msg->getAddress();
  ...
  if (hit_where == HitWhere::DRAM)
    hit_where = (sender == shmem_msg->getRequester()) ? HitWhere::DRAM_LOCAL
  ...
  getMemoryManager()->sendMsg(
      ...
      shmem_req->getShmemMsg()->getRequester() /* receiver */, address,
      shmem_msg->getDataBuf(), getCacheBlockSize(), hit_where,
      shmem_req->getShmemMsg()->getPerf(), ShmemPerfModel::_SIM_THREAD);
...
void DramDirectoryCntlr::processInvRepFromL2Cache(core_id_t sender,
                                                  ShmemMsg* shmem_msg) {
  IntPtr address = shmem_msg->getAddress();
  ...
  DirectoryBlockInfo* directory_block_info =
      directory_entry->getDirectoryBlockInfo();
  LOG_ASSERT_ERROR(
      directory_block_info->getDState() == DirectoryState::SHARED ||
          directory_block_info->getDState() == DirectoryState::EXCLUSIVE,
      "Ooops (%lx)", address);
    ...
    if (shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::EX_REQ) {
      ...
      if (directory_block_info->getDState() == DirectoryState::UNCACHED) {
        ...
        processExReqFromL2Cache(shmem_req);
               ...
               ShmemMsg::UPGRADE_REQ) {
      MYLOG("as part of UPGR: got INV_REP from %d, %d sharers left", sender,
            directory_entry->getNumSharers())

      // An ShmemMsg::UPGRADE_REQ caused the invalidation: additional action is
      // only required
      // when all SHARED copies have been invalided
      if (directory_entry->getNumSharers() == 1) {
        core_id_t requester = shmem_req->getShmemMsg()->getRequester();
        if (directory_entry->hasSharer(requester)) {
          // all Shared copies have been invalidated, UPGRADE_REQ can be
          // completed now.
          MYLOG(
              "UPGRADE_REQ: all OTHER shared copies have been invalided: "
              "complete upgrade req.\n");
          updateShmemPerf(shmem_req, ShmemPerf::INV_IMBALANCE);
          processUpgradeReqFromL2Cache(shmem_req);
        }
      } else if (directory_entry->getNumSharers() == 0) {
        MYLOG(
            "UPGRADE_REQ: all shared copies have been invalided, requester has "
            "no copy anymore.\n");
        ...
        MYLOG(
            "could not do complete UPGRADE_REQ because there are still "
            "multiple sharers");
      }
    } else if (shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::SH_REQ) {
      if (shmem_req->getWaitForData()) {
        // This is a voluntary invalidate (probably part of an upgrade or
        // eviction),
        // the next request should only be woken up once its data arrives from
        // DRAM.
      } else if (shmem_req->isForwarding() &&
                 sender != shmem_req->getForwardingFrom()) {
        // This is a voluntary invalidate (probably part of an upgrade or
        // eviction),
        // the next request should only be woken up once its data arrives from
        // the forwarder.
...
void DramDirectoryCntlr::processUpgradeReqFromL2Cache(ShmemReq* shmem_req,
                                                      Byte* cached_data_buf) {
  ...
  MYLOG("processUpgradeReqFromL2Cache for address: %lx, requester= %d", address,
        requester);
  DirectoryEntry* directory_entry =
      m_dram_directory_cache->getDirectoryEntry(address);
  if (directory_entry == NULL) {
  ...
  switch (curr_dstate) {
    ...
    case DirectoryState::MODIFIED: {
      if (sharers_list_pair.second[0] == requester) {
        MYLOG("upgrade request immediately finished, sending UPGRADE_REP to %d",
              requester);
        assert(directory_entry->getOwner() == requester);

        getMemoryManager()->sendMsg(
            ShmemMsg::UPGRADE_REP, MemComponent::TAG_DIR,
            MemComponent::L2_CACHE, requester /* requester */,
            requester /* receiver */, address, NULL, 0, HitWhere::UNKNOWN,
        ...
        MYLOG("FLUSH REQ (UPGR)>%u @ %lx", sharers_list_pair.second[0],
              address);
        getMemoryManager()->sendMsg(
            ShmemMsg::FLUSH_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
            requester /* requester */,
            directory_entry->getOwner() /* receiver */, address, NULL, 0,
            HitWhere::UNKNOWN, shmem_msg->getPerf(),
            ShmemPerfModel::_SIM_THREAD);
      }
    ...
    case DirectoryState::SHARED: {
      if ((sharers_list_pair.second.size() == 1) &&
          (sharers_list_pair.second[0] == requester)) {
        ...
        if (!requesterHasCopy) {
          MYLOG(
              "UPGRADE_REQ: %lu sharer(s), but the requester isn't holding a "
              "copy!?",
              sharers_list_pair.second.size());
        }
        ...
        if (sharers_list_pair.first == true) {
          ...
          for (UInt32 i = 0; i < sharers_list_pair.second.size(); i++) {
            if (sharers_list_pair.second[i] != requester) {
              MYLOG("INV REQ (UPGR)>%u @ %lx", sharers_list_pair.second[i],
                    shmem_msg->getAddress());
              // avoid having to fetch the data from DRAM, so ask at least one
              // core to FLUSH instead of INV
              ShmemMsg::msg_t msg_type = (!requesterHasCopy && i == 0)
                                             ? ShmemMsg::FLUSH_REQ
                                             : ShmemMsg::INV_REQ;
              // ShmemMsg::msg_t msg_type = ShmemMsg::INV_REQ;
              getMemoryManager()->sendMsg(
                  msg_type,  // ShmemMsg::INV_REQ,
...
void DramDirectoryCntlr::processFlushRepFromL2Cache(core_id_t sender,
                                                    ShmemMsg* shmem_msg) {
  IntPtr address = shmem_msg->getAddress();
  SubsecondTime now =
      getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD);
  ...
  // could be that this is a FLUSH to force a core with S-state to to write back
  // clean data
  ...
  if (m_dram_directory_req_queue_list->size(address) != 0) {
    ...
    if (shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::EX_REQ) {
      ...
      processShReqFromL2Cache(shmem_req, shmem_msg->getDataBuf());
               ...
               ShmemMsg::UPGRADE_REQ) {
      MYLOG("as part of UPGR: got FLUSH_REP from %d, %d sharers left", sender,
            directory_entry->getNumSharers());
      // there should be only one M copy that is written back
      if (directory_entry->getNumSharers() == 0) {
        // The only M copy has been invalidated (WB), UPGRADE_REQ can be
        // completed now.
        directory_block_info->setDState(DirectoryState::UNCACHED);
        processUpgradeReqFromL2Cache(shmem_req, shmem_msg->getDataBuf());
      } else {
        // This is part of a UPGRADE REQ where the requester did not have a copy
        // anymore
        // One of the sharers whas FLUSHed instead of INV to get the data. This
        // is it's reply
...
void DramDirectoryCntlr::processWbRepFromL2Cache(core_id_t sender,
                                                 ShmemMsg* shmem_msg) {
  IntPtr address = shmem_msg->getAddress();
  SubsecondTime now =
      getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD);
...
void DramDirectoryCntlr::sendDataToDram(IntPtr address, core_id_t requester,
  ...
  getMemoryManager()->sendMsg(
      PrL1PrL2DramDirectoryMSI::ShmemMsg::DRAM_WRITE_REQ, MemComponent::TAG_DIR,
      MemComponent::DRAM, requester /* requester */, dram_node /* receiver */,
      address, data_buf, getCacheBlockSize(), HitWhere::UNKNOWN, NULL,
      ShmemPerfModel::_SIM_THREAD);
