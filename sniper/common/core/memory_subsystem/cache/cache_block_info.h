...
// Cannot include compress_utils.h because this would create a circular
// reference
...
class CacheBlockInfo {
 public:
  ...
  // Track usage on 1<<BitsUsedOffset granularity (per 64-bit / 8-byte)
  static const UInt8 BitsUsedOffset = 3;

  // Enough to store one bit per 1<<BitsUsedOffset byte element per cache
  // line (8 8-byte elements for 64-byte cache lines)
 ...
 public:
  CacheBlockInfo(IntPtr tag                  = static_cast<IntPtr>(~0),
                 CacheState::cstate_t cstate = CacheState::INVALID,
                 UInt64 options              = 0);
  virtual ~CacheBlockInfo();

  static std::unique_ptr<CacheBlockInfo> create(CacheBase::cache_t cache_type);
