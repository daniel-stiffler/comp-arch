...
const char* CacheBlockInfo::getOptionName(option_t option) {
  static_assert(CacheBlockInfo::NUM_OPTIONS ==
                    sizeof(CacheBlockInfo::option_names) / sizeof(char*),
                "Not enough values in CacheBlockInfo::option_names");
...
CacheBlockInfo::CacheBlockInfo(IntPtr tag, CacheState::cstate_t cstate,
                               UInt64 options)
    : m_tag(tag), m_cstate(cstate), m_owner(0), m_used(0), m_options(options) {}

CacheBlockInfo::~CacheBlockInfo() {}

std::unique_ptr<CacheBlockInfo> CacheBlockInfo::create(
    CacheBase::cache_t cache_type) {

  switch (cache_type) {
    case CacheBase::PR_L1_CACHE:
      return std::unique_ptr<CacheBlockInfo>(new PrL1CacheBlockInfo());

    case CacheBase::PR_L2_CACHE:
      return std::unique_ptr<CacheBlockInfo>(new PrL2CacheBlockInfo());

    case CacheBase::SHARED_CACHE:
      return std::unique_ptr<CacheBlockInfo>(new SharedCacheBlockInfo());
...
bool CacheBlockInfo::updateUsage(UInt32 offset, UInt32 size) {
  UInt64 first      = offset >> BitsUsedOffset,
         ...
         first_mask = (1ull << first) - 1, last_mask = (1ull << (last + 1)) - 1,
...
bool CacheBlockInfo::updateUsage(BitsUsedType used) {
  bool new_bits_set =
      used & ~m_used;  // Are we setting any bits that were previously unset?
